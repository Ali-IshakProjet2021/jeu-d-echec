...
public class Joueur{
	...
	public Piece bougerT1(Position depart) {					
		...
		for (Piece count: tab)
		{
			 if (depart.equals(count.getPosition()))
			 {
				 selectionnee=count;
				 ok=true;
				 System.out.println(count.getMouvementExecutable());
				 break;
			 }
		...
		if (!ok)
			System.out.println("position erronnÈe, rejouer");
		return selectionnee;
	...
	public Piece bougerRoiT1(Position depart) {					
		...
		if (depart.equals(Roi.getRoiCouleur(couleur).getPosition()))
		{
			...
			ok=true;
			System.out.println(selectionnee.getMouvementExecutable());
	...
	public boolean bougerT2(Piece selectionnee, Position depart,Position arrivee) {
		...
		{			
			...
			System.out.println("erreur d'arrivÈ");
		return tentative;			
	}
